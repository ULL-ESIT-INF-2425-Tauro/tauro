...
export function CartButton({ cartItems, isEditMode }: CartButtonProps) {
  ...
  return (
    ...
    >
      {isEditMode ? (
        <div className="flex items-center p-1.5 sm:p-2 rounded-full hover:bg-white/10 transition-colors cursor-not-allowed">
      ...
      <CartPreview cartItems={cartItems} isOpen={isCartPreviewOpen} isEditMode={isEditMode} />
    </div>
  );
}
